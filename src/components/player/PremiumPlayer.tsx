...
interface PremiumPlayerProps {
  ...
  /** All available servers shown in the server-switch dropdown — each entry is
   * independently selectable (cinemaos sources are never grouped/collapsed). */
  availableServers?: {
    id: string;
    name: string;
    available: boolean;
    format: "hls" | "mp4" | "dash";
    languages: string[];
    /** Background reachability probe is currently in flight for this entry */
    checking?: boolean;
    /** Background reachability probe (or a real playback attempt) failed */
    bad?: boolean;
    quality?: string;
    speed?: string | null;
    /** Upstream API's own "verified" hint — distinct from our live probe result */
    verified?: boolean;
  }[];
  /** Called when user picks a server from the dropdown */
  onServerSwitch?: (id: string) => void;
  /** Explicit stream container type — avoids fragile URL sniffing when the
   * caller already knows it (e.g. cinemaos sources report their own type).
   * "auto" preserves the existing sniffing behavior. */
  streamType?: "hls" | "dash" | "mp4" | "auto";
  /** Fires once real playback is confirmed reachable (manifest parsed / can-play) */
  onPlaybackReady?: () => void;
  /** scraperNames currently being checked in the background, shown as a
   * cycling label over the buffering spinner while true */
  checkingServerNames?: string[];
}
...
function isDashUrl(streamUrl: string): boolean {
  if (streamUrl.includes(".mpd")) return true;
  try {
    const u = new URL(streamUrl, window.location.href);
    for (const [, value] of u.searchParams) {
      if (value.includes(".mpd")) return true;
    }
  } catch {
    // ignore
  }
...
/** Resolves the explicit `streamType` prop when given, otherwise sniffs the
 * URL — kept as one function so every effect that needs to know the
 * container type (init, stall watchdog) agrees on the same answer. */
function resolveStreamType(
  streamUrl: string,
  explicit: "hls" | "dash" | "mp4" | "auto",
): "hls" | "dash" | "mp4" {
  if (explicit !== "auto") return explicit;
  if (isDashUrl(streamUrl)) return "dash";
  if (isHlsUrl(streamUrl)) return "hls";
  return "mp4";
}
...
const PremiumPlayer = forwardRef<{ seek: (time: number) => void }, PremiumPlayerProps>((
    ...
    const onPlaybackReadyRef = useRef<PremiumPlayerProps["onPlaybackReady"]>(onPlaybackReady);
    ...
    const applyPendingSeek = useCallback(() => {
      ...
      if (video.readyState < 1) return; // not ready yet — a later ready event retries

      const target = Math.max(0, t);
      const doSeek = () => {
        // DASH is backed by MediaSource Extensions — dash.js's own seek()
        // asks it to fetch the right segments first, which is more reliable
        // than poking `currentTime` directly before that timestamp is known.
        if (dashRef.current) {
          try {
            dashRef.current.seek(target);
            return;
          } catch {
      ...
      // A seek attempted this early can silently not "stick" (MSE hasn't
      // buffered that far yet) — verify it actually landed and retry with
      // backoff a few times before giving up, instead of losing it forever.
      let attempts = 0;
      const verify = () => {
        const v = videoRef.current;
        if (!v || pendingSeekRef.current == null) return;
        if (Math.abs(v.currentTime - target) < 2) {
          pendingSeekRef.current = null;
          return;
        }
        attempts++;
        ...
        doSeek();
        setTimeout(verify, 400 * attempts);
      };
    ...
    }, [onError, onTimeUpdate, onComplete, onPlaybackReady]);

    // Cycle through "Checking {server}..." names over the buffering spinner
    // while a nova candidate is being validated in the background.
    const [checkingNameIdx, setCheckingNameIdx] = useState(0);
    useEffect(() => {
      if (checkingServerNames.length === 0) return;
      const t = setInterval(() => setCheckingNameIdx((i) => (i + 1) % checkingServerNames.length), 1100);
      return () => clearInterval(t);
    }, [checkingServerNames.length]);
    ...
    useEffect(() => {
      ...
      const resolvedType = resolveStreamType(streamUrl, streamType);

      if (resolvedType === "dash") {
        let destroyed = false;
        import("dashjs").then((dashjs) => {
          ...
          player.updateSettings({
            streaming: { retryAttempts: { MPD: 2, MediaSegment: 2 } },
          });
          ...
          player.on(dashjs.MediaPlayer.events.ERROR, (e: unknown) => {
            ...
            reportSourceFailure(`DASH error: ${JSON.stringify(e).slice(0, 200)}`);
          });
        });
    ...
    useEffect(() => {
      const video = videoRef.current;
      // HLS (hls.js) and DASH (dashjs) each report their own fatal errors —
      // this watchdog is only for true progressive/mp4 sources.
      if (!video || !streamUrl || resolveStreamType(streamUrl, streamType) !== "mp4") return;
      ...
      >
        ...
        {isLoading && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/30 z-20">
            ...
            {checkingServerNames.length > 0 && (
              <p
                key={checkingNameIdx}
                className="absolute top-[calc(50%+3rem)] left-1/2 -translate-x-1/2 whitespace-nowrap text-xs font-semibold tracking-wide text-white/70 animate-in fade-in duration-300"
              >
                Checking {checkingServerNames[checkingNameIdx % checkingServerNames.length]}...
              </p>
            )}
          </div>
        )}
          ...
          {showServerIcon && activeServerInfo && (
            <div className="relative">
              ...
              {showServerDropdown && availableServers.length > 0 && (
                <div
                  className="absolute top-full right-0 mt-2 w-64 sm:w-72 bg-black/95 backdrop-blur-2xl rounded-2xl border border-white/10 shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200"
                  onClick={(e) => e.stopPropagation()}
                >
                  <div className="p-3 border-b border-white/5 bg-white/5 flex items-center justify-between">
                    <p className="text-[10px] font-black text-white/40 uppercase tracking-[0.2em]">Switch Server</p>
                    {(() => {
                      const checkable = availableServers.filter(
                        (s) => s.checking || s.bad || s.available,
                      );
                      if (checkable.length === 0) return null;
                      const finished = availableServers.filter((s) => s.available || s.bad).length;
                      return (
                        <p className="text-[9px] font-bold text-white/30 tabular-nums">
                          {finished}/{availableServers.length} checked
                        </p>
                      );
                    })()}
                  </div>
                  <div className="p-1.5 max-h-[360px] overflow-y-auto custom-scrollbar">
                    {availableServers.map((s) => {
                      const isActive = s.name === activeServerInfo.name;
                      const dotClass = s.checking
                        ...
                        <button
                          ...
                          className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all duration-200 text-left
                            ${isActive
                              ? "bg-white/10 text-white"
                              : s.bad
                                ? "opacity-50 hover:bg-white/5 text-white/50 hover:text-white/80"
                                : "hover:bg-white/5 text-white/60 hover:text-white"}`}
                        >
                          <div className={`w-2 h-2 rounded-full flex-shrink-0 ${dotClass}`} />
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className="font-bold text-sm tracking-tight truncate">{s.name}</span>
                              <span className="text-[9px] font-black px-1.5 py-0.5 rounded bg-white/10 text-white/50 flex-shrink-0">
                                {s.format.toUpperCase()}
                              </span>
                              {s.quality && (
                                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-white/5 text-white/40 flex-shrink-0">
                                  {s.quality}
                                </span>
                              )}
                              {s.speed && (
                                <span className="text-[9px] font-bold text-amber-300/70 flex-shrink-0">{s.speed}</span>
                              )}
                            </div>
                          ...
                          {isActive && <Check className="w-3.5 h-3.5 text-white/60 flex-shrink-0" />}
                        </button>
                      );
                    })}
                  </div>
                ...
                <div className="relative">
                  ...
                  {showCaptions && (
                    <div
                      ...
                      onClick={(e) => e.stopPropagation()}
                    >
                      <div className="p-1.5 max-h-[40vh] overflow-y-auto custom-scrollbar">
                        <button
