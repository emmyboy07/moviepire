...
//
// NOTE: these guards cannot save you from an OOM kill. The kernel's cgroup
// OOM-killer sends SIGKILL, which is uncatchable - if RestartCount is climbing
// while these handlers are installed, the cause is memory, not an exception.
// Fix OOM by reducing SEGMENT_CACHE_MAX_BYTES and setting --max-old-space-size
// in your start command (see nixpacks.toml).
//
...
// A coalesced fetch either produced a buffered (cacheable) segment, or was too
// large / of unknown size to buffer, in which case the live Response is handed
// to exactly one caller and streamed straight through.
type SegmentResult =
  | { kind: "buffered"; entry: CachedSegment }
  | { kind: "stream"; response: Response; claimed: boolean };

const segmentCache = new Map<string, CachedSegment>();
const inFlightSegmentFetches = new Map<string, Promise<SegmentResult>>();
const SEGMENT_CACHE_TTL_MS = 6_000; // roughly one segment duration

// FIX: Reduced from 500 → 200 entries and from 192 MB → 64 MB to prevent
// OOM kills. The old defaults (192 MB cache + Node.js heap + React SSR) could
// easily push a 256–512 MB container over its cgroup memory limit, causing the
// kernel to send SIGKILL every ~30 s as the cache refilled. These values are
// safe for containers with 256 MB+; increase via env vars for larger instances.
const SEGMENT_CACHE_MAX_ENTRIES = 200;
const SEGMENT_CACHE_MAX_BYTES = Number(process.env.SEGMENT_CACHE_MAX_BYTES || 64 * 1024 * 1024);

// Hard ceiling on buffering ANY single upstream response into memory. Anything
// larger (or of unknown length) is streamed through instead of materialised.
// Reduced from 24 MB → 8 MB: with 10+ concurrent in-flight fetches the old
// value could spike 240 MB of transient RAM before the cache ceiling even
// applied. 8 MB comfortably covers a standard 2-6s HLS segment.
const SEGMENT_MAX_BUFFER_BYTES = Number(process.env.SEGMENT_MAX_BUFFER_BYTES || 8 * 1024 * 1024);

let segmentCacheBytes = 0;

function segmentCacheDelete(key: string) {
  const existing = segmentCache.get(key);
  if (!existing) return;
  segmentCacheBytes -= existing.body.byteLength;
...
function evictSegmentCache() {
  while (
    segmentCache.size > 0 &&
    (segmentCache.size > SEGMENT_CACHE_MAX_ENTRIES || segmentCacheBytes > SEGMENT_CACHE_MAX_BYTES)
  ) {
    const oldest = segmentCache.keys().next().value;
    if (oldest === undefined) break;
    segmentCacheDelete(oldest);
  }
  if (segmentCache.size === 0) segmentCacheBytes = 0; // guard against drift
}

function segmentCacheSet(key: string, entry: CachedSegment) {
  if (entry.body.byteLength > SEGMENT_MAX_BUFFER_BYTES) return;
  segmentCacheDelete(key);
  segmentCache.set(key, entry);
  segmentCacheBytes += entry.body.byteLength;
  evictSegmentCache();
}

// Runs on every segment request. Unlike the previous version this always drops
// expired entries (the old early-return meant stale buffers were never freed
// below the entry cap) and always enforces both ceilings.
function pruneSegmentCache() {
  const now = Date.now();
  for (const [key, val] of segmentCache) {
    if (val.expires <= now) segmentCacheDelete(key);
  }
  evictSegmentCache();
}
...
// Some upstream CDNs - and, it turns out, some proxy layers depending on the
// runtime's fetch()/http implementation - silently ignore a Range request and
// return the full file with 200 instead of a partial 206. When that happens
// the browser's <video> element can only play forward from byte 0, which is
// exactly what "can't seek" looks like. This slices the response ourselves
// from the stream (never buffering the whole file into memory, and cancelling
// the upstream read as soon as the wanted bytes are emitted) whenever the
// client asked for a range but didn't get one back.
async function enforceRange(response: Response, rangeHeader: string | null): Promise<Response> {
  if (!rangeHeader || response.status === 206 || !response.ok) return response;

  const match = /^bytes=(\d+)-(\d*)$/.exec(rangeHeader);
  if (!match) return response;

  const totalStr = response.headers.get("content-length");
  if (!totalStr || !response.body) return response;

  const total = Number(totalStr);
  const start = Number(match[1]);
  const end = match[2] ? Number(match[2]) : total - 1;
  if (!Number.isFinite(total) || !Number.isFinite(start) || start >= total) return response;
  const safeEnd = Math.min(end, total - 1);
  const wantedLength = safeEnd - start + 1;
  ...
  const sliced = new ReadableStream<Uint8Array>({
    async pull(controller) {
      ...
      // Entirely before the requested range - drop and wait for the next chunk.
      if (position <= start) return;
      if (chunkStart < start) chunk = chunk.subarray(start - chunkStart);

      // Trim anything that overshoots the requested range.
      const remaining = wantedLength - emitted;
      ...
      if (emitted >= wantedLength) {
        controller.close();
        // Stop pulling more than the client asked for - saves bandwidth on
        // early seeks into a large file instead of downloading the rest.
  ...
  headers.set("content-length", String(wantedLength));
  ...
  if (url.pathname === "/api/proxy-stream") {
    ...
    try {
      ...
      if (looksLikeM3u8) {
        ...
        response = await fetchWithRetry();
        response = await enforceRange(response, range);
      } else {
        ...
        if (cached && cached.expires > Date.now()) {
          ...
          if (!pending) {
            pending = (async (): Promise<SegmentResult> => {
              const r = await fetchWithRetry();

              // The single most important guard in this file. `isFullMediaFile`
              // only matches a known video extension on the path; a full movie
              // served from an extensionless or aliased CDN URL falls through to
              // here, and arrayBuffer()-ing it pulls the entire file into RAM.
              // Unknown length is treated as "too big" - we refuse to gamble.
              const declared = Number(r.headers.get("content-length") ?? Number.NaN);
              if (!Number.isFinite(declared) || declared > SEGMENT_MAX_BUFFER_BYTES) {
                return { kind: "stream", response: r, claimed: false };
              }
              ...
              if (r.status === 200 && !range) segmentCacheSet(cacheKey, entry);
              return { kind: "buffered", entry };
            })();
            inFlightSegmentFetches.set(cacheKey, pending);
            // `.finally()` returns a NEW promise that rejects when `pending`
            // rejects, with nobody attached - that was firing a spurious
            // unhandled rejection on every failed upstream segment fetch.
            // Swallow first, then clean up.
            void pending
              .catch(() => undefined)
              .finally(() => {
          ...
          if (result.kind === "buffered") {
            response = new Response(result.entry.body as BodyInit, {
              status: result.entry.status,
              headers: result.entry.headers,
            });
          } else if (!result.claimed) {
            // A streamed body can only be consumed once, so the first waiter
            // takes it and any coalesced waiters fall back to their own fetch.
            result.claimed = true;
            inFlightSegmentFetches.delete(cacheKey);
...
// Returns the echo-back origin, or null when the request origin isn't allowed.
// There is no fallback origin - an unknown origin simply gets no CORS headers,
// which is what a browser should see for a cross-origin request we don't intend
// to permit. A wildcard isn't usable here because Allow-Credentials is set.
  ...
  async fetch(request: Request, env: unknown, ctx: unknown) {
    try {
      ...
      if (new URL(request.url).pathname === "/health") {
        ...
        return new Response(
          JSON.stringify({
            ...
            // Surfaced so a climbing RSS is visible BEFORE the OOM-killer acts.
            // rss is the number the kernel's cgroup limit is compared against.
            memoryMb: {
              rss: Math.round(mem.rss / 1048576),
              heapUsed: Math.round(mem.heapUsed / 1048576),
              external: Math.round(mem.external / 1048576),
              arrayBuffers: Math.round((mem.arrayBuffers ?? 0) / 1048576),
            },
            segmentCache: {
              entries: segmentCache.size,
              bytesMb: Math.round(segmentCacheBytes / 1048576),
              inFlight: inFlightSegmentFetches.size,
            },
          }),
          {
            status: 200,
      ...
      if (request.method === "OPTIONS") {
        ...
        const allowOrigin = getCorsOrigin(reqOrigin);
        ...
        if (allowOrigin) {
          ...
          preflightHeaders["Access-Control-Allow-Headers"] = "Content-Type, Authorization, x-request-nonce";
      ...
      // Without this line the exception is silently discarded and the only
      // visible symptom is the generic error page - log it so `docker logs`
      // / Coolify's Logs tab shows the real cause and stack trace.
      console.error("[server] request handler threw:", error);
      return new Response(renderErrorPage(), {
        status: 500,
        headers: { "content-type": "text/html; charset=utf-8" },
      });
