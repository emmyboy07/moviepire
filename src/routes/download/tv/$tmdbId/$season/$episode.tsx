...
import { loadTv } from "@/lib/media.functions";
import { z } from "zod";
...
export const Route = createFileRoute("/download/tv/$tmdbId/$season/$episode")({
  validateSearch: (search) => downloadSearchSchema.parse(search),
  head: () => ({
    meta: [
      { title: "Download Series Episode — Moviepire" },
      { name: "description", content: "Download TV episodes instantly in any quality." },
    ],
  }),
  component: SeriesDownload,
});
...
// Content-Disposition with the title/season/episode/resolution baked in.
function buildDownloadUrl(q: Quality, title: string, season: number, episode: number): string {
  const filename = `${title} S${season}E${episode} (${q.resolution}p).${q.format || "mp4"}`.replace(/[\\/:*?"<>|]/g, "");
...
function buildSubtitleDownloadUrl(sub: Caption, title: string, season: number, episode: number): string {
  const ext = sub.url.split(".").pop()?.split("?")[0] || "srt";
  const filename = `${title} S${season}E${episode} - ${sub.label}.${ext}`.replace(/[\\/:*?"<>|]/g, "");
...
function SeriesDownload() {
  ...
  useEffect(() => {
    if (document.getElementById("ad-crowdsynonym")) return;
    const s = document.createElement("script");
    s.id = "ad-crowdsynonym";
    s.src = "https://crowdsynonym.com/e4/19/52/e419524a9a1241d37f12356470a5cbda.js";
    document.body.appendChild(s);
  }, []);

  useEffect(() => {
    if (document.getElementById("ad-97239")) return;
    const s = document.createElement("script");
    s.id = "ad-97239";
    s.setAttribute("data-cfasync", "false");
    s.async = true;
    s.type = "text/javascript";
    s.src = "//monkismtheban.com/gWkkDB6cMNLBw9/97239";
    document.body.appendChild(s);
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const resp = await fetchTv({
          data: { tmdbId: id, season: parsedSeason, episode: parsedEpisode },
        });
        if (!active) return;
        if (!resp.ok) {
          setError(resp.error || "Failed to load series download details");
        } else {
    ...
    return (
      <div className="flex h-screen w-screen flex-col items-center justify-center bg-[#0a0a0a] px-6 text-center">
        ...
        <p className="mt-2 max-w-md text-sm text-white/60">
          {error || "Could not retrieve download links for this episode. Please verify the TMDB ID, season, and episode."}
        </p>
      </div>
    );
  }
        ...
        <div className="mt-10 flex w-full flex-wrap justify-center gap-4">
          {qualities.length === 0 ? (
            ...
            qualities.map((q) => (
              <a
                key={q.id}
                href={buildDownloadUrl(q, result.title, parsedSeason, parsedEpisode)}
                target="_blank"
                rel="noreferrer"
                download
            ...
            {showSubs && (
              <div className="mt-5 space-y-2">
                {captions.map((sub, i) => (
                  <a
                    key={i}
                    href={buildSubtitleDownloadUrl(sub, result.title, parsedSeason, parsedEpisode)}
                    target="_blank"
                    rel="noreferrer"
                    download
