...
import { loadMovie } from "@/lib/media.functions";
import { z } from "zod";
...
export const Route = createFileRoute("/download/movie/$tmdbId")({
  validateSearch: (search) => downloadSearchSchema.parse(search),
  head: () => ({
    meta: [
      { title: "Download Movie — Moviepire" },
      { name: "description", content: "Download movies instantly in any quality." },
    ],
  }),
  component: MovieDownload,
});
...
// Content-Disposition with the title/resolution baked in.
function buildDownloadUrl(q: Quality, title: string): string {
  const filename = `${title} (${q.resolution}p).${q.format || "mp4"}`.replace(/[\\/:*?"<>|]/g, "");
...
function buildSubtitleDownloadUrl(sub: Caption, title: string): string {
  const ext = sub.url.split(".").pop()?.split("?")[0] || "srt";
  const filename = `${title} - ${sub.label}.${ext}`.replace(/[\\/:*?"<>|]/g, "");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const resp = await fetchMovie({ data: { tmdbId: id } });
        if (!active) return;
        if (!resp.ok) {
          setError(resp.error || "Failed to load movie download details");
        } else {
    ...
    return (
      <div className="flex h-screen w-screen flex-col items-center justify-center bg-[#0a0a0a] px-6 text-center">
        ...
        <p className="mt-2 max-w-md text-sm text-white/60">
          {error || "Could not retrieve download links for this movie. Please verify the TMDB ID."}
        </p>
      </div>
    );
  }
        ...
        <div className="mt-10 flex w-full flex-wrap justify-center gap-4">
          {qualities.length === 0 ? (
            ...
            qualities.map((q) => (
              <a
                key={q.id}
                href={buildDownloadUrl(q, result.title)}
                target="_blank"
                rel="noreferrer"
                download
            ...
            {showSubs && (
              <div className="mt-5 space-y-2">
                {captions.map((sub, i) => (
                  <a
                    key={i}
                    href={buildSubtitleDownloadUrl(sub, result.title)}
                    target="_blank"
                    rel="noreferrer"
                    download
